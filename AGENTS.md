...
| Date | Task | Details |
|---|---|---|
| 09-07 | **[DONE] FIX: Emmaus report competitor duplicates + report query filters merged** | Two-part fix for the Emmaus report's duplicate competitor list. **(1) Report query fix**: `_emmaus_build_report.py` — added `(status IS NULL OR status != 'merged')` to BOTH competitor queries (market competition by faith + nearest Protestant list) so soft-merged records are excluded. **(2) Merged 2 remaining duplicate pairs** the global dedup missed: `_fix_emmaus_dupes.py --write` — RIVERBEND (id=5101346 sba_ppp → canonical 3526218 holy_sites) + SAINT ANDREWS (id=4972178 osm → canonical 539439 overture). **Why the global dedup missed them**: (a) RIVERBEND pair is `sba_ppp↔holy_sites` — source pair NOT in option-2 scope (only holy_sites↔osm + overture↔osm); (b) SAINT ANDREWS pair is `overture↔osm` (in scope) but names differ after exact normalization (`SAINT ANDREWS PRESBYTERIAN` vs `St. Andrews Presbyterian Church` — SAINT/ST + CHURCH suffix), so the audit's exact-name match missed it. Both soft-merged per convention (oldest survives, `status='merged'` + `dedup_canonical_id` + `merged_into` log + rollback map). Provenance `dedup_emmaus_report_dupes`. Regenerated report: competitor list now 25 unique entries, no duplicates; competition counts 111/512/1185 at 5/10/15km. **BROADER FINDING**: 45 unmerged duplicate pairs within 15km of Emmaus remain (irs+holy_sites↔osm, sba_ppp↔osm, holy_sites↔holy_sites internal, cog_anderson_api internal, overture↔sba_ppp, etc.) — the global dedup only covered 2 source-pair patterns + exact-name matching. `church_infrastructure` same-faith counts (76@5km) are pre-computed and NOT updated by this fix. |
| 09-06 | **[DONE] PUBLISH: Independent Christian Church blog post** | `scripts/outreach/publish_icc_post.py` — published "The Church That Refuses to Be a Denomination: Inside the Independent Christian Churches" to Substack (gridkeeper) as draft 214501933. Data-rich post on the instrumental branch of the Restoration Movement (Stone-Campbell). Covers: frontier origins (Barton Stone, Thomas/Alexander Campbell), the 3-way split (a cappella Churches of Christ / Disciples of Christ / Independent Christian Churches), the instrumental-vs-a cappella organ question, the "[City] Christian Church" naming convention (never named after people), the Christian Standard magazine (Cincinnati, 1866) connection, and the Midwest heartland. GRID data: 4,121 ICC (taxonomy 324) vs 4,335 CoC (320) vs 2,641 Disciples (318); ~midwest% in IN/IL/KY/OH/MO/KS/OK/MI/IA/MN/WI/NE/ND/SD; top states IN 364, IL 306, CA 245, KY 241, OH 226, MO 202, FL 201, KS 150, OK 138, TX 137. Includes a **grouped bar chart by state** showing all three branches in red (ICC) / blue (CoC) / green (Disciples) — updated via `--draft-id 214501933` (script supports updating existing drafts). Temp scripts `_blog_icc_data.py`/`_blog_icc_data2.py` cleaned up. |
| 09-06 | **[DONE] MERGE: Christian Standard Directory — 7,439 reclassified + 550 new** | `_merge_christian_standard.py` — imported Christian Standard Directory (christianstandard.com/directory, WordPress directory plugin) into churches.db. Staging: `data/staging/christian_standard.db` (8,300 listings crawled: christianchurch 4,328, churchofchrist 3,837, missionorganization 135). **Reclassified 7,439** existing GRID churches into correct Restoration Movement taxonomy nodes via GPS proximity dedup (cKDTree, 500m, `WHERE status IS NULL OR status != 'merged'` — NOT status='active' which excludes 4.4M NULL-status churches). **Inserted 550** genuinely new churches (IDs 7055456–7056005): 313 Independent Christian Church (324), 219 Church of Christ (320), 18 Mission Organization (tax=None, landmark_type='organization'). Noise filter excluded 3 non-religious (Emergency Dentist Houston, Lenna's Ladybugs, Nestack Reviews and Ratings). Each new church gets churches + church_location (state/zip/country) + church_addresses + address_components (city) + church_contact_values (phone/facebook). Provenance `christian_standard_directory_2026` (7,439 upd / 550 ins, completed) + 22,867 enrichment_change_log entries. **GOTCHAS**: (1) `church_location` has NO city column — city lives in `address_components` (component_type='city'); (2) gw_db.Connection wrapper requires `db._conn.row_factory = sqlite3.Row` (setting `db.row_factory` doesn't propagate); (3) most GRID churches have status=NULL not 'active'. |
| 09-07 | **[DONE] DEDUP: 65,850 global duplicate records soft-merged (option 2)** | `_fix_dupes_global.py --write` — fixed the two dominant cross-source overlap patterns: `holy_sites_import`↔`osm_import` (45,789 pairs) + `overture_full`↔`osm_import` (19,345 pairs). Pairs from `data/dupe_pairs_global.json` (305,455 total, filtered to 65,864 target pairs) grouped into 64,490 connected-component clusters (1,267 are 3-way+). **Merge convention honored**: oldest record (lowest `churches.id`) survives as canonical; newer duplicate soft-merged via `status='merged'` + `dedup_canonical_id=<canonical>` + `merged_into` enrichment_change_log entry. **Soft-merge (NOT hard delete)** — duplicates keep all enrichment rows (church_location/addresses/etc.), nothing lost; queries filter `status != 'merged'` or follow `dedup_canonical_id`. Rollback map written to `churches_dedup_rollback` (65,850 new rows → 147,260 total). Provenance `dedup_global_option2` (65,850 updated, completed). Verified: 65,850 `merged_into` log entries, 39,317 merged records retain church_location, 22,906 retain church_addresses, 0 canonicals marked merged. **GOTCHA**: `last_updated` is unreliable (NULL/inconsistent formats) — use `churches.id` (lower = older) to determine canonical survivor. |
| 09-07 | **[DONE] IMPORT: 42,440 NRAA religious sites** | `_import_nraa_religious_sites.py --write` — imported all 42,440 NRAA (National Religious Affairs Administration) religious sites from `api.sara.gov.cn` into staging DB `data/staging/nraa_religious_sites.db`. Buddhism 34,089 | Taoism 8,351. All 42,440 have addresses, region codes, and person-in-charge data. **TECHNICAL FIX**: Rewrote `import_all()` to properly enclose all Playwright browser context code inside `with sync_playwright() as p:` block — previous version had broken indentation causing "Event loop is closed!" error. Uses `page.evaluate()` with `fetch()` to bypass Python TLS issues with api.sara.gov.cn. **NEXT**: Geocode addresses via AMap API (requires API key), then spatially merge into churches.db. |
| 09-06 | **[DONE] REPORT: Emmaus Church (Columbia SC) full catchment report** | `outputs/emmaus_report.txt` — single-church report for Emmaus Church (id=3334312, 5605 Bush River Rd, Columbia SC, tract 45063021109). Tract-FIPS-based catchment models (5km=11, 10km=38, 15km=77 tracts) aggregated from ACS 2023 5-year via Census API (`data/census/emmaus_acs_tract.json`). Sections: church profile, catchment models, demographic subcategories (families w/children, ethnic mix, age, education), economic data (income, home values, poverty, employment, housing tenure), FEMA NRI risk (flood/wildfire/tornado/hurricane/earthquake), infrastructure proximity, market competition (117/522/1211 sites at 5/10/15km by faith + nearest Protestant list), and 13 grayscale B&W heat maps (`outputs/emmaus_report/maps/`). Key figures: 5km pop 40,379, med income $77,725, med home $212,361, families w/children 31.5%, white 61.1%/black 26.8%, poverty 9.1%, NRI risk 40.3. Pipeline: `_emmaus_build_report.py` (aggregate) → `_emmaus_maps.py` (tricontourf heat maps) → `_emmaus_write_report.py` (report .txt). |
| 09-06 | **[DONE] MERGE: 69 Numlookup reverse-phone city fills** | `_merge_numlookup_city.py` — 69 US churches (phone, no addr, no zip, no GPS) got city fills from Numlookup reverse-phone API (100% city coverage). Created 69 `church_addresses` rows (address_type='primary', is_current=1, source='numlookup', country='US') + 69 `address_components` city components. **BUG FIXED**: `church_addresses.address_id` is a regular INT (not auto-populating) — merge left it NULL; `_fix_numlookup_addr_id.py` backfilled `address_id=rowid` for all 69 rows (0 NULL remaining). **PROVENANCE FIXED**: `Provenance.__exit__` INSERTs provenance after the in-block `db.commit()` so it was never committed — added `db.commit()` after the `with` block in both scripts + backfilled 2 rows via `_backfill_numlookup_provenance.py` (sources `numlookup_reverse_phone_2026`, `numlookup_addr_id_fix_2026`, 69 churches each). Verified: 69 addr rows, 69 city components, 207 change-log entries, 2 provenance entries. Numlookup quota exhausted (100/mo). **NEXT**: IPQS daily runs (35/day) for zip + phone-activity validation. |
