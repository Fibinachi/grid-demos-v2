#!/usr/bin/env python3
"""Generate a report of the 25 neediest Methodist churches in Alabama."""
import sqlite3, os
...
# Use the family field for consistent Methodist classification
total = c.execute("SELECT COUNT(x) FROM (SELECT 1 as x FROM churches WHERE state = 'AL' AND family = 'Methodist Churches')").fetchone()[0]
with_data = c.execute("SELECT COUNT(x) FROM (SELECT 1 as x FROM churches WHERE state = 'AL' AND family = 'Methodist Churches' AND acs_poverty_rate IS NOT NULL)").fetchone()[0]

# Also show national for context
natl = c.execute("SELECT COUNT(x) FROM (SELECT 1 as x FROM churches WHERE family = 'Methodist Churches')").fetchone()[0]
print(f'Methodist Churches family — National: {natl:,}  |  Alabama: {total:,}  |  With ACS data: {with_data:,}')

# Show denom breakdown for context
c.execute("SELECT denomination, COUNT(x) as cnt FROM (SELECT denomination, 1 as x FROM churches WHERE state = 'AL' AND family = 'Methodist Churches') GROUP BY denomination ORDER BY cnt DESC")
print('\nDenominational breakdown:')
for r in c.fetchall():
    print(f'  {r[0] or "(blank)"}: {r[1]}')
...
md = []
md.append(f'# Alabama Methodist Church Needs Assessment Report')
md.append(f'')
md.append(f'**Generated:** {now}')
md.append(f'')
md.append(f'**Scope:** {comma(total)} Methodist churches in Alabama ({comma(with_data)} with tract-level ACS data)')
md.append(f'')
md.append(f'---')
md.append(f'')
...
md.append(f'The top {TOP_N} neediest Methodist churches in Alabama serve communities with:')
...
md.append(f'---')
md.append(f'')
md.append(f'## Top {TOP_N} — Ranked by Need')
md.append(f'')
md.append(f'| # | Church | City | Denomination | Poverty | Income | Unempl. | Score |')
md.append(f'|---|--------|------|-------------|---------|--------|---------|-------|')
for i, r in enumerate(rows, 1):
    name = r['name'][:40] if r['name'] else '?'
    city = r['city'][:15] if r['city'] else ''
    d = (r['denomination'] or '?')[:20]
    md.append(f'| {i} | {name} | {city} | {d} | {pct(r["acs_poverty_rate"])} | {dollar(r["acs_median_income"])} | {pct(r["acs_unemployment_rate"])} | {r["need_score"]} |')
...
for i, r in enumerate(rows, 1):
    ...
    md.append(f'')
    md.append(f'**Location:** {fmt(r["address"])}, {fmt(r["city"])}, AL {fmt(r["zip"])}')
    md.append(f'')
...
path = os.path.join(OUT_DIR, 'al_methodist_neediest_report.md')
...
for i, r in enumerate(rows[:5], 1):
    print(f'  {i}. {r["name"]} — {r["city"]} — {r["denomination"]} — {pct(r["acs_poverty_rate"])} poverty')
