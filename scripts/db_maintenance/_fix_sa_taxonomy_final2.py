"""Fix all SA entries taxonomy - row by row."""
import sqlite3
...
c = db.execute("""
    ...
    WHERE religion_type = 'christian'
      AND tradition_legacy = 'Protestant'
      AND family = 'Holiness Churches'
      AND denomination_affiliation = 'Salvation Army'
      AND (name LIKE '%Salvation Army%' OR name LIKE '%Ejercito de Salvacion%')
""").fetchone()[0]
