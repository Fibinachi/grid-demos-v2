"""
Rebuild ahmadiyya_hierarchy with Baitul Futuh (Morden, London) as the true world HQ.
"""
import sqlite3
...
def main():
    ...
    # Identify World HQ: Baitul Futuh Mosque, Morden (has city data)
    world_hq = None
    for r in rows:
        nu = (r[2] or "").upper()
        if "BAITUL FUTUH" in nu and (r[3] or ""):
            world_hq = r
            print(f"World HQ: rowid={r[0]} | {r[2]} | {r[3]}, {r[5]}")
            break
    if not world_hq:
        for r in rows:
            nu = (r[2] or "").upper()
            if "BAITUL FUTUH" in nu:
                world_hq = r
                print(f"World HQ (no city): rowid={r[0]} | {r[2]} | {r[5]}")
                break
    if not world_hq:
        for r in rows:
            nu = (r[2] or "").upper()
            if r[5] == "GB" and ("LONDON MOSQUE" in nu or "FAZL MOSQUE" in nu):
                world_hq = r
                print(f"World HQ (Fazl Mosque): rowid={r[0]} | {r[2]} | {r[5]}")
                break
    if not world_hq:
        uk = [r for r in rows if r[5] == "GB"]
        if uk:
            world_hq = uk[0]
            print(f"World HQ (fallback): rowid={world_hq[0]} | {world_hq[2]}")
    ...
    """)
    print("Created ahmadiyya_hierarchy table")
    ...
    hq_id = ins(None, world_hq[1], world_hq[2], 'hq',
                ...
                None, None,
                "Ahmadiyya Muslim Community global headquarters - London, UK")
    print(f"World HQ: id={hq_id} | {(world_hq[2] or '').encode('ascii', errors='replace').decode('ascii')}")
