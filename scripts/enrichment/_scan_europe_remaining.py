"""Re-scan remaining Europe entries not covered by the crashed scan."""
...
SCRIPT_NAME = "scan_europe_jewish_deepseek_remaining"
...
# Get entries NOT already changed by the first scan
c.execute(f"SELECT id, name, city, state, country, landmark_type, tradition FROM churches WHERE faith='Judaism' AND country IN ({ph}) ORDER BY country, name", europe)
entries = c.fetchall()
total_before = len(entries)
print(f"Europe entries still to scan: {total_before}")

# Count already-scanned entries
c.execute("SELECT COUNT(DISTINCT church_id) FROM enrichment_change_log WHERE change_source='deepseek_europe_scan'")
already_scanned = c.fetchone()[0]
print(f"Already scanned in first pass: {already_scanned}")
...
for cs in range(0, len(entries), CHUNK_SIZE):
    ...
    print(f"  Batch {bn}/{tb} ({len(items)} entries)...", end=" "); sys.stdout.flush()
...
notes = f"DeepSeek Europe Judaism scan (round 2): {total_moved} moved out, {total_retyped} retyped."
