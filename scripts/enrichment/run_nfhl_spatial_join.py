"""
NFHL Spatial Join v2 — BBOX-first approach.
Pre-computed bounding boxes (compute_nfhl_bboxes.py) enable fast pre-filtering.
...
class BBoxIndex:
    """Spatial index over bounding boxes with lazy WKB deserialization."""
    ...
    def add(self, min_x, min_y, max_x, max_y, zone, subty, sfha, wkb):
        self.bboxes.append((min_x, min_y, max_x, max_y))
        self.zone_data.append((zone, subty, sfha, wkb))
    
    def build(self):
        arr = np.array(self.bboxes, dtype=np.float64)
        self.min_x = arr[:, 0]
        self.min_y = arr[:, 1]
        self.max_x = arr[:, 2]
        self.max_y = arr[:, 3]
        self.sort_idx = np.argsort(self.min_x)
        self._sorted = True
        print(f"  Index built: {len(self.bboxes):,} polygons")
    
    def query_bboxes(self, x, y):
        """Return indices of polygons whose bbox contains (x, y)."""
        right = np.searchsorted(self.min_x[self.sort_idx], x, side='right')
        candidates = []
        for j in range(right):
            i = self.sort_idx[j]
            if self.max_x[i] >= x and self.min_y[i] <= y <= self.max_y[i]:
                candidates.append(i)
        return candidates
    
    def check_containment(self, idx, x, y):
        """Check if polygon at idx contains point. Returns zone info or None."""
        from shapely import wkb as shapely_wkb
        from shapely.geometry import Point
        
        wkb = self.zone_data[idx][3]
        if len(wkb) > MAX_WKB_DESERIALIZE:
            b = self.bboxes[idx]
            if b[0] <= x <= b[2] and b[1] <= y <= b[3]:
                return self.zone_data[idx][:3]
            return None
        try:
...
print("=== NFHL SPATIAL JOIN v2 (BBOX-first) ===\n")
...
for i, row in enumerate(rows):
    index.add(row[0], row[1], row[2], row[3], row[4], row[5], row[6], row[7])
    if (i+1) % 100000 == 0:
...
print("\nStep 2: Building spatial index...")
t0 = time.time()
index.build()
print(f"  Built in {time.time()-t0:.0f}s")

# Step 3: Load US churches from main DB
...
# Step 4: BBOX-first spatial join
print("\nStep 4: BBOX-first classification...")
...
for i in range(0, len(churches), CHUNK_SIZE):
    ...
    for church_id, lat, lon in batch:
        candidates = index.query_bboxes(lon, lat)
        ...
        for idx in candidates:
            wkb_deserialized += 1
            zone_info = index.check_containment(idx, lon, lat)
            if zone_info:
                z, s, sfha = zone_info
...
# Step 5: Write results to work DB (main DB locked)
print("\nStep 5: Writing results to work DB church_enrichment...")
db_write = sqlite3.connect(str(WORK_DB), timeout=120)
...
db_summary = sqlite3.connect(str(WORK_DB), timeout=60)
