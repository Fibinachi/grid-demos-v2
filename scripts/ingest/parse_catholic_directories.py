"""
Parser v2: State-machine extraction of clergy assignments from Catholic Directory
machine-formatted files. Handles multi-line clergy continuations.

Pattern: City, County, Church — Rev. Name, Role; Rev. Name2, Role2.

Usage:
  python scripts/ingest/parse_catholic_directories.py --year 1860 --dry-run
  python scripts/ingest/parse_catholic_directories.py --year 1860
  python scripts/ingest/parse_catholic_directories.py --all
  python scripts/ingest/parse_catholic_directories.py --stats
"""
...
# ── Diocese header ─────────────────────────────────────────────────
RE_DIOCESE = re.compile(r'^(?:ARCH)?DIOCESE\s+OF\s+(.+?)\.?\s*$', re.IGNORECASE)

# ── Church + clergy line: has dash separator ──────────────────────
RE_CHURCH_LINE = re.compile(r'^(.+?)\s*[—\-]\s*(.+)$')

# ── Structural lines to skip ──────────────────────────────────────
STRUCTURAL_PATS = [
    ...
    r'^BISHOP\.?\s*$', r'^ARCHBISHOP\.?\s*$', r'^ARCHBISHOP\.?\s*$',
...
def parse_clergy_text(text):
    """Parse clergy text into structured records."""
    priests = []
    ...
    while i < len(parts) - 1:
        ...
        if name_match:
            ...
            remainder = name_part[name_match.end():].strip().lstrip(',').lstrip(';').strip().rstrip('.').rstrip(',').rstrip(';').strip()

            order = None
            for o in ORDERS:
                o_pat = re.escape(o).replace(r'\.', r'\.\s*')
                if re.search(o_pat, remainder):
                    order = o.rstrip('.')
                    remainder = re.sub(o_pat, '', remainder, flags=re.IGNORECASE).strip().lstrip(',').lstrip(';').strip()
                    break
...
def extract_church_parts(text):
    """Extract city and church name from church identification text."""
    ...
    if parts:
        ...
        if not is_church:
            if not re.match(r'^(?:corner|between|near|Front|East|West|North|South|attended|served|who|also|High|Vespers|Residence|Mass)', first, re.IGNORECASE):
...
def parse_file(filepath, year):
    """Parse a directory file. Returns list of assignment dicts."""
    records = []
    current_diocese = None
    in_church_section = False  # Only parse church data after CHURCHES AND CLERGY header

    with open(filepath, 'r', encoding='utf-8', errors='replace') as f:
        raw = f.read()

    # Preprocess: join hyphenated words split across lines
    # "Ober-\nmyer" → "Obermyer"
    raw = re.sub(r'([a-z])-\s*\n\s*([a-z])', r'\1\2', raw)
    ...
    while i < len(lines):
        ...
        if dio_match:
            ...
            in_church_section = False  # Reset — wait for CHURCHES AND CLERGY
            i += 1
            continue

        # Detect church section boundary
        ...
        if church_match:
            ...
            for p in priests:
                records.append({
                    ...
                    'source_line': line_num, 'raw_line': line, 'clergy_text': clergy_part,
                })
            i = j
            continue

        # Fallback: line has "Rev." but no dash
...
GARBAGE_PARISH = re.compile(
    r'^$'                           # Empty
    r'|^(Most|Rt\.?|Very|V\.|Mt\.?)$'  # Title fragments
    ...
    r'|^\d+$'
    r'|^.{1,3}$'                    # 1-3 chars
    r'|^.{0,40},\s*(?:S\.\s*J\.|C\.\s*S\.\s*S\.|O\.\s*S\.|C\.\s*M\.|D\.\s*D\.|Sup|V\.\s*G\.|Pastor|Rector|Chaplain)'
    , re.IGNORECASE
)


def filter_garbage(records):
    """Remove records with garbage parish names."""
...
def create_tables(db):
    db.execute("""
        CREATE TABLE IF NOT EXISTS catholic_clergy (
            ...
            year INTEGER NOT NULL, diocese TEXT, city TEXT, parish TEXT,
            priest_name TEXT NOT NULL, title TEXT, religious_order TEXT,
            role TEXT, church_id INTEGER, source_file TEXT,
            source_line INTEGER, raw_line TEXT, clergy_text TEXT,
    ...
    db.execute("""
        CREATE TABLE IF NOT EXISTS catholic_parishes (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            year INTEGER NOT NULL, diocese TEXT, city TEXT,
            parish_name TEXT NOT NULL, num_clergy INTEGER DEFAULT 0,
            source_file TEXT, created_at TEXT DEFAULT (datetime('now'))
        )
    """)
    for idx in ['year', 'priest_name', 'diocese', 'parish']:
        db.execute(f"CREATE INDEX IF NOT EXISTS idx_cclergy_{idx} ON catholic_clergy({idx})")
    db.commit()


def parse_all_years(db, dry_run=False):
    files = sorted(DIRECTORIES.glob('[12][0-9][0-9][0-9]_formatted.txt'))
        ...
        try:
            ...
            if not dry_run and recs:
                for j in range(0, len(recs), 500):
                    chunk = recs[j:j+500]
                    db.executemany("""INSERT OR IGNORE INTO catholic_clergy
                        (year, diocese, city, parish, priest_name, title, religious_order, role, source_file, source_line, raw_line, clergy_text)
                        VALUES (?,?,?,?,?,?,?,?,?,?,?,?)""",
                        [(r['year'],r['diocese'],r['city'],r['parish'],r['priest_name'],
                          r['title'],r['order'],r['role'],r['source_file'],r['source_line'],r['raw_line'],r['clergy_text'])
                         for r in chunk])
                db.commit()
            total += len(recs)
            if recs:
                for r in recs[:3]:
                    print(f"    {r['diocese'] or '?'} | {r['parish'] or '?'} | {r['priest_name']} | {r['role'] or '-'}")
...
def main():
    ...
    ap.add_argument('--limit', type=int, default=0, help='Limit to N files (for testing)')
    args = ap.parse_args()
    ...
    if args.stats:
        rows = db.execute("""
            SELECT year, COUNT(*) n, COUNT(DISTINCT priest_name) priests,
                   COUNT(DISTINCT parish) parishes, COUNT(DISTINCT diocese) dioceses
            FROM catholic_clergy GROUP BY year ORDER BY year
        """).fetchall()
        if rows:
            print(f"{'Year':<6} {'Assignments':>12} {'Priests':>10} {'Parishes':>10} {'Dioceses':>10}")
            print("-"*50)
            for r in rows:
                print(f"{r[0]:<6} {r[1]:>12,} {r[2]:>10,} {r[3]:>10,} {r[4]:>10,}")
        else:
            print("No data yet. Run with --year first.")
        return

    if args.year:
        fp = DIRECTORIES / f"{args.year}_formatted.txt"
        if not fp.exists():
            print(f"Not found: {fp}"); return
        recs = parse_file(fp, args.year)
        print(f"\n{len(recs)} assignments from {args.year}")
        for r in recs[:20]:
            print(f"  {r['diocese'] or '?'} | {r['city'] or '?'} | {r['parish'] or '?'} | {r['priest_name']} | {r['role'] or '-'}")
        if not args.dry_run and recs:
            db.executemany("""INSERT OR IGNORE INTO catholic_clergy
                ...
                [(r['year'],r['diocese'],r['city'],r['parish'],r['priest_name'],
                 ...
                 for r in recs])
            db.commit()
            print(f"[OK] {len(recs)} inserted")
    elif args.years:
        for y in args.years:
            fp = DIRECTORIES / f"{y}_formatted.txt"
            if not fp.exists(): print(f"Not found: {fp}"); continue
            recs = parse_file(fp, y)
            print(f"{y}: {len(recs)}")
            if not args.dry_run and recs:
                db.executemany("""INSERT OR IGNORE INTO catholic_clergy
                    (year,diocese,city,parish,priest_name,title,religious_order,role,source_file,source_line,raw_line,clergy_text)
        ...
        parse_all_years(db, args.dry_run)
    else:
        ap.print_help()
    db.close()
